...
- `bg_main_opa`（lv_opa_t）指定渐变第一个颜色的透明度。默认值：255。
- `bg_grad_opa`（lv_opa_t）指定渐变第二个颜色透明度。默认值：255。
...
|        blend_mode        |   渲染模式   |    lv_blend_mode_t    | LV_BLEND_MODE_NORMAL | False | False |  False   |
